...
const findMatch2 = (sum, reportArr) => {
  
  const sortedReport = reportArr.sort((a, b) => a - b);

  for (let num of sortedReport) {
    ...
    while (selector1.index !== selector2.index) {
      if (selector1.value + selector2.value + num === sum) {
        return [selector1, selector2, num];
      }
      if (selector1.value + selector2.value + num > sum) {
        selector2.index --;
...
console.log(findMatch2(2020, report));